...
- **FR-009**: CAPT MUST refuse a destination whose non-following `lstat` mode is not a regular file, even when `--force` is present. The predicate is `stat.S_ISREG` on `os.lstat` (or the platform equivalent). It is not an enumerated list of special types and MUST NOT use follow-symlink helpers such as `Path.is_file()`. Missing (`ENOENT`) is the only destination that may be created without `--force`. Error messages MAY distinguish symlink and directory; every other non-regular mode uses the not-a-regular-file reason.
