...
- Deterministic insight rules for repeated tool calls and repeated failed tool calls

Out of scope today:

- Additional insight rules (loops, oversized outputs, compactions, subagent analysis)
...
3. **Domain.** Deterministic insight rules operate on summaries and normalized records. Repeated tool calls and repeated failed tool calls are implemented. Future rules may detect loops, redundant calls, oversized outputs, and later support a Context Ledger.
