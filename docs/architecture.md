...
3. **Domain.** Deterministic insight rules operate on summaries and normalized records. Repeated tool calls, repeated failed tool calls, high cumulative tool result volume, high tool failure rate, and dominant tool usage are implemented. Future rules may detect loops, redundant calls, oversized individual outputs, and later support a Context Ledger.
