...
- Deterministic compact handoff export from one explicit local capture or `--latest`, using a closed capture-source allowlist and a versioned JSON v1 contract independent from summary and comparison schemas, with optional explicit `--output` file persistence
