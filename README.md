...
> **Early-stage project.** CAPT is in pre-alpha. This version can collect raw Claude Code OpenTelemetry envelopes locally. It does not yet normalize events, analyze sessions, or produce performance insights.
...
This version does **not** normalize OTLP payloads, inspect Claude Code transcripts, persist data in a database, or generate insights.
...
Metrics use `"signal":"metrics"` and the received `resourceMetrics` object as `payload`. Normalization and analysis are not implemented yet.
